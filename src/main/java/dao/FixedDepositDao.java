...
public class FixedDepositDao {
	...
	// Why this ONE method now exists, used by both processMaturedDeposits()
	// and prematureWithdrawFD(): previously each method had its OWN copy of
	// this formula, written independently — one used no explicit rounding
	// scale, the other used scale=2/HALF_UP. Two independently-maintained
	// copies of the same formula is exactly how they silently drifted apart.
	// Extracting this ensures both methods always compute interest with
	// IDENTICAL rounding behavior, no matter which one is edited later.
	//
	// Why "years" is a BigDecimal parameter here, not an int: this lets the
	// SAME method serve both callers even though they arrive at "years"
	// completely differently — processMaturedDeposits() passes the whole
	// number of years the FD promised (e.g. 3), while prematureWithdrawFD()
	// passes a precise FRACTIONAL year (e.g. 0.667 for 8 months held).
	private BigDecimal calculateSimpleInterest(BigDecimal principal, BigDecimal ratePercent, BigDecimal years) {
		return principal.multiply(ratePercent).multiply(years).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
	}
	...
	// succeed together, or you'd end up with money missing but no FD to show for
	// it.
	public boolean bookFD(FixedDeposit fd, boolean testMode) {
		...
		try {
			...
			LocalDateTime maturityDateTime = testMode ? bookDateTime.plusMinutes(fd.getNoOfYears() * 5L)
	...
	public void processMaturedDeposits() {
		...
		try (Connection conn = ConnectionFactory.getConnection();
			...
			while (rs.next()) {
				...
				// Why we pass the STATED no_of_years here (as a BigDecimal),
				// not actual elapsed real-world time: a MATURED FD has, by
				// definition, run its complete promised term — whether that
				// term was measured in real years or test-mode minutes, the
				// customer is entitled to the FULL agreed interest. Contrast
				// with prematureWithdrawFD() below, which passes a
				// FRACTIONAL elapsed-time value instead, because that FD did
				// NOT complete its agreed term. Both now go through the SAME
				// calculateSimpleInterest() method, so the arithmetic and
				// rounding are guaranteed identical — only the "years" input
				// differs, and it differs deliberately, not accidentally.
				BigDecimal interest = calculateSimpleInterest(principal, rate, BigDecimal.valueOf(years));
	...
	public BigDecimal prematureWithdrawFD(int fdId, String accno) {
		...
		try {
			...
			BigDecimal yearsHeld = BigDecimal.valueOf(daysHeld).divide(BigDecimal.valueOf(365.0), 10,
			...
			// Why this now calls the SAME shared method as
			// processMaturedDeposits(), passing yearsHeld (a FRACTIONAL
			// value) instead of a whole number: this FD is closing before
			// its agreed term, so interest is prorated to actual time held —
			// but the underlying formula and rounding rule is now
			// guaranteed identical to the maturity path, eliminating the
			// silent divergence that existed before (no rounding scale here
			// vs. scale=2 there).
			BigDecimal interest = calculateSimpleInterest(principal, effectiveRate, yearsHeld);
